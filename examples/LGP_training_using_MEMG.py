...
from rl_adn.environments.env import IESEnv, MEMG_config
...
def process(funcs, env):
    ...
    info_steps = [None] * time_split_num
...
def evaluate_test(individual, env, toolbox):
    ...
    info_file = os.path.join(env.log_dir, 'infor_longterm.txt')
    fitness_file = os.path.join(env.log_dir, 'fitness_longterm.txt')
    action_file = os.path.join(env.log_dir, 'action_longterm.txt')
...
def find_best_population_parallel(env, population, toolbox, max_workers):
    ...
    args = [(individual, toolbox, env, train_day_num) for individual in population]
...
class MultiInputLGPIndividual(list):
    ...
    def _get_arg_value(self, arg_type, idx, inputs, registers, const_tiled, m):
        """
        Helper to retrieve the value of an argument based on its type and index.

        Args:
            arg_type: str, one of 'input', 'register', 'constant'.
            idx: int, the index for the type.
...
def main(args_input):
    ...
    # if args_input.isEncoding:
    #     if args_input.is_long_term_eval:
    #         env_args['env_name'] = 'Encoding-7days/MEMG'
    #     else:
    #         env_args['env_name'] = 'Encoding_result/MEMG'
    # else:
    #     if args_input.is_long_term_eval:
    ...
    if args.train:
        args.if_remove = True
        args.cwd = f"./{env_args['env_name']}/{env_args['data_path']}/{args_input.agent_class}/{args_input.run_name}"
    ...
    if args.train:
        ...
        population, log = algorithms.eaWithBest_parallel(
            ...
            pset=None, max_workers=max_workers, max_height=max_height, stats=stats, start_time=start_time_laurent,
            verbose=True
        )
...
def parse_args():
    ...
    parser.add_argument('--run_name', type=str, default='MEMG-Remote-TEST', help='Name of the run')
    ...
    parser.add_argument('--program_length', type=int, default=180)
    parser.add_argument('--regis_num', type=int, default=16)
    return parser.parse_args()


if __name__ == "__main__":
    run_names = ['MEMG-short-term-12h']
