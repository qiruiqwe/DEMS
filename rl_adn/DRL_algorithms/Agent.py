...
class AgentBase:
    ...
    def __init__(self, net_dims: [int], state_dim: int, action_dim: int, discrete_value: Tensor, gpu_id: int = 0, args: Config = Config()):
        ...
        self.if_probability_discrete = args.if_probability_discrete
        self.discrete_value = discrete_value

        self.state_dim = state_dim
        self.action_dim = action_dim  # 指构件数目
        ...
        if self.if_probability_discrete:
            self.act = self.act_target = act_class(net_dims, state_dim, action_dim, self.discrete_value).to(self.device)
            self.cri = self.cri_target = cri_class(net_dims, state_dim, action_dim, self.discrete_value).to(self.device) \
...
class AgentDDPG(AgentBase):
    ...
    def update_net(self, buffer: ReplayBuffer) -> Tuple[float, ...]:
        ...
        for update_c in range(update_times):
            ...
            obj_critics += obj_critic.item()  # q_value 和 q_label 的差距
            self.optimizer_update(self.cri_optimizer, obj_critic)
            self.soft_update(self.cri_target, self.cri, self.soft_update_tau)
            '''compare with TD3, DDPG no policy delay update'''
            action_pg = self.act(state)  # policy gradient
            obj_actor = self.cri_target(
                torch.cat((state, action_pg), dim=1)).mean()  # use cri_target is more stable than cri
            obj_actors += obj_actor.item()  # actor中所有动作的平均值
    ...
    def explore_one_env(self, env, horizon_len: int, if_random: bool = False) -> [Tensor]:
        ...
        actions = torch.zeros((horizon_len, self.num_envs, self.action_dim), dtype=torch.float32).to(self.device)
        rewards = torch.zeros((horizon_len, self.num_envs), dtype=torch.float32).to(self.device)
        dones = torch.zeros((horizon_len, self.num_envs), dtype=torch.bool).to(self.device)  # 结束标志存储
