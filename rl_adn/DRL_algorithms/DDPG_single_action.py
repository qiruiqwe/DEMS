...
class Actor(nn.Module):
    ...
    def get_action(self, state: Tensor) -> Tensor:
        ...
        """
        with torch.no_grad():
            single_action = self.net(state).tanh().squeeze()
            single_noise = (torch.randn_like(single_action) * self.explore_noise_std).clamp(-0.5, 0.5)
        return (single_action + single_noise).clamp(-1.0, 1.0)
...
class AgentDDPG_single(AgentBase):
    """
    ...
    def __init__(self, net_dims: [int], state_dim: int, action_dim: int, discrete_value: NDArray[np.float32], gpu_id: int = 0, args: Config = Config()):
        ...
        super().__init__(net_dims=net_dims, state_dim=state_dim, action_dim=action_dim,
                         discrete_value=self.discrete_value, gpu_id=gpu_id, args=args)

        self.act = self.act_class(net_dims, state_dim, 1).to(self.device)
        ...
        self.cri = self.cri_class(net_dims, state_dim, 1).to(self.device)
        self.cri.discrete_value = self.discrete_value
        self.cri.discrete_value_num = len(discrete_value)
        self.cri.action_dim = self.action_dim
    ...
    def get_obj_critic_raw(self, buffer: ReplayBuffer, batch_size: int) -> Tuple[Tensor, Tensor]:
        ...
        q_values = self.cri(torch.cat((states, actions[:, 0].unsqueeze(1)), dim=1))
    ...
    def explore_one_env(self, env, horizon_len: int, if_random: bool = False) -> [Tensor]:
        ...
        for i in range(horizon_len):
            ...
            if if_random:
                single_action = torch.rand(1) * 2 - 1.0
            else:
                single_action = get_action(state.unsqueeze(0))
            action = single_action.repeat(self.action_dim)
