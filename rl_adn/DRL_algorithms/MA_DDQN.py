...
from rl_adn.DRL_algorithms.DDQN import AgentDDQN
...
class AgentMADDQN:
    ...
    def __init__(self, net_dims: [int], state_dim: int, action_dim: int, discrete_value: NDArray[np.float32],
        ...
        self.agents = [
            AgentDDQN(net_dims, state_dim, action_dim, discrete_value, gpu_id, args) for _ in range(self.num_agents)
        ]
    ...
    def select_action(self, state: np.ndarray, epsilon) -> int:
        ...
        actions = torch.zeros((self.num_agents, 1))
        for i, agent in enumerate(self.agents):
            action_idx = agent.select_action(state, epsilon)
            actions[i] = action_idx  # Only take scalar
        return actions
    ...
    def explore_env(self, env, horizon_len: int, if_random: bool = False) -> Tuple[Tensor, ...]:
        ...
        for t in range(horizon_len):
            # 1. 动作索引选择（不含离散动作转换）
            epsilon = 1.0 if if_random else self.epsilon
            action_indices = self.select_action(ary_state, epsilon)  # shape: (num_agents, 1)

            # 2. 记录状态与动作索引（原始整数动作）
            states_buf[t] = torch.as_tensor(ary_state, dtype=torch.float32, device=self.device)
            actions_buf[t] = torch.as_tensor(action_indices, dtype=torch.float32, device=self.device)

            # 3. 转为环境需要的连续动作值
            action_indices = action_indices.to(torch.int64).view(-1).cpu().numpy()
            discrete_actions = self.discrete_value[action_indices]  # shape: (num_agents,)
            next_state, reward, done, _ = env.step(discrete_actions)
