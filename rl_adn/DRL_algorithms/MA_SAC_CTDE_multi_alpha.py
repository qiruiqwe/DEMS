...
class AgentMASAC_CTDE_MAL(AgentBase):
    """
    Centralized Training with Decentralized Execution, CTDE
    ...
    def __init__(self, net_dims: [int], state_dim: int, action_dim: int, discrete_value: NDArray[np.float32], gpu_id: int = 0, args: Config = Config()):
        ...
        # 独立 alpha_log 和优化器
        self.alpha_logs = [torch.tensor(-1.0, dtype=torch.float32, requires_grad=True, device=self.device)
                           for _ in range(self.agent_nums)]
        self.alpha_optimizers = [torch.optim.Adam([alpha_log], lr=self.learning_rate)
                                 for alpha_log in self.alpha_logs]
        self.target_entropy = np.log(action_dim)
        # self.target_entropy_per_agent = -1.0  # 每个代理的目标熵（动作维度为 1）
    ...
    def update_net(self, buffer: ReplayBuffer) -> Tuple[float, ...]:
        ...
        for _ in range(update_times):
            ...
            for i in range(self.agent_nums):
                # 生成联合动作，第 i 个代理需要梯度
                next_actions_tensor_i = torch.zeros(self.batch_size, self.agent_nums).to(self.device)
                next_logprobs_tensor_i = torch.zeros(self.batch_size, self.agent_nums).to(self.device)
                for j in range(self.agent_nums):
                    if j == i:
                        ...
                        with torch.no_grad():
                            next_action_j, next_logprob_j = self.acts[j].get_action_logprob(state)
                    next_actions_tensor_i[:, j] = next_action_j.squeeze(-1)
                    next_logprobs_tensor[:, j] = next_logprob_j.squeeze(-1)

                alpha_i = self.alpha_logs[i].exp().detach()
                q_actor_i = self.cri(state, next_actions_tensor_i)
                obj_actor_i = (alpha_i * next_logprobs_tensor_i[:, i] - q_actor_i).mean()
                obj_actors_iter += obj_actor_i.item()
            ...
            obj_actors += obj_actors_iter / self.agent_nums

            '''objective of alpha'''
            for i in range(self.agent_nums):
                obj_alpha_i = (self.alpha_logs[i] * (
                            -next_logprobs_tensor[:, i].detach() + self.target_entropy)).mean()
                self.optimizer_update(self.alpha_optimizers[i], obj_alpha_i)
                alphas += self.alpha_logs[i].exp().detach().item() / self.agent_nums
    ...
    def get_obj_critic_raw(self, buffer, batch_size: int) -> Tuple[Tensor, Tensor]:
        ...
        with torch.no_grad():
            ...
            alpha = torch.mean(torch.stack([self.alpha_logs[i].exp() for i in range(self.agent_nums)]))
