...
class ActorSAC(nn.Module):
    ...
    def __init__(self, dims: [int], state_dim: int, action_dim: int, discrete_value: Tensor):
        ...
        self.dec_a = build_mlp(dims=[dims[-1], action_dim*self.discrete_num])  # decoder of action mean
    ...
    def forward(self, state: Tensor) -> Tensor:
        ...
        logits = self.dec_a(state_tmp)  # Shape: (batch_size, action_dim * discrete_num)
        probs = torch.softmax(logits.view(-1, self.action_dim, self.discrete_num),
                              dim=-1)  # Shape: (batch_size, action_dim, discrete_num)
        action_indices = torch.argmax(probs, dim=-1)  # Shape: (batch_size, action_dim)
        actions = self.discrete_value[action_indices]  # Map indices to discrete values
        return actions  # Shape: (batch_size, action_dim)
    ...
    def get_action_logprob(self, state: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        logits = self.dec_a(state_tmp)
        probs = torch.softmax(logits.view(-1, self.action_dim, self.discrete_num), dim=-1)  # Shape: (batch_size, action_dim, discrete_num)

        # Sample actions
        action_indices = torch.multinomial(probs.view(-1, self.discrete_num), 1).view(-1, self.action_dim)  # Shape: (batch_size, action_dim)
        actions = self.discrete_value[action_indices]  # Shape: (batch_size, action_dim)

        # Compute log probabilities
        log_probs = torch.log(probs + 1e-8)  # Add small epsilon to avoid log(0)
        selected_log_probs = log_probs.gather(-1, action_indices.unsqueeze(-1)).squeeze(-1)  # Shape: (batch_size, action_dim)
        log_prob_sum = selected_log_probs.sum(dim=-1, keepdim=True)  # Shape: (batch_size, 1)
...
class AgentPSAC(AgentBase):
    ...
    """
    def __init__(self, net_dims: [int], state_dim: int, action_dim: int, discrete_value: NDArray[np.int32], gpu_id: int = 0, args: Config = Config()):
        self.device = torch.device(f"cuda:{gpu_id}" if (torch.cuda.is_available() and (gpu_id >= 0)) else "cpu")
        self.discrete_value = torch.from_numpy(discrete_value).to(torch.int32).to(self.device)
        self.discrete_num = discrete_value.shape[0]  # 离散值数目
        ...
        super().__init__(net_dims, state_dim, action_dim, self.discrete_value, gpu_id, args)
        ...
        # Target entropy for discrete action space: log(number of discrete values) per component
        self.target_entropy = np.log(self.act.discrete_num) * action_dim
    ...
    def explore_one_env(self, env, horizon_len: int, if_random: bool = False) -> [Tensor]:
        ...
        for i in range(horizon_len):
            ...
            if if_random:
                # Random discrete actions from discrete_value
                action_indices = torch.randint(0, self.act.discrete_num, (self.action_dim,), device=self.device)
                action = self.act.discrete_value[action_indices]
            else:
                action = get_action(state.unsqueeze(0))[0][0]  # Shape: (action_dim,)
