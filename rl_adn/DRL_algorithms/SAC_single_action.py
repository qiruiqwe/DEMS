...
class ActorSAC(nn.Module):
    ...
    def get_action(self, state: Tensor) -> Tensor:
        ...
        with torch.no_grad():
            ...
            action = (action_avg + action_std * noise).clamp(-1.0, 1.0)
        return action.squeeze()  # action (re-parameterize)
...
class AgentSAC_single(AgentBase):
    """
    ...
    def get_obj_critic_raw(self, buffer, batch_size: int) -> Tuple[Tensor, Tensor]:
        ...
        q1, q2 = self.cri.get_q1_q2(torch.cat((states, actions[:, 0].unsqueeze(1)), dim=1))
