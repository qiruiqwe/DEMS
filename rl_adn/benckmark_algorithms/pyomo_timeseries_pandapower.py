...
def construct_opf_model(Vnom, Vmin, Vmax, Data_Network):
    ...
    model.SOC = Var(model.TIMES, model.NODES, initialize=model.battery_initial_soc,
    ...
    model.energy_change = Var(model.TIMES, model.NODES, initialize=energy_change_rule,
    ...
    def active_power_flow_rule(model, time, k):
        ...
        return (sum(model.P[time, (j, i)] for j, i in model.LINES if i == k) - sum(
            model.P[time, (i, j)] + model.RM[i, j] * (model.I[time, (i, j)] ** 2) for i, j in model.LINES if k == i) +
                model.PS[time, k] + model.energy_change[time, k] ==
                model.PD[time, k])
