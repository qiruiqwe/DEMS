...
class Battery():
    ...
    def __init__(self, parameters):
        ...
        self.is_split = parameters['is_split']
    ...
    def step(self, action_battery):
        ...
        """
        # 1) 改为两个离散动作{-1, 1}，DRL优于GP
        # action_battery = np.where(action_battery < 0, -1, np.where(action_battery > 0, 1, action_battery))
        # 2) 改为三个离散动作
        if self.is_split:
            threshold = 1 / 3
            conditions = [
                action_battery < -threshold,
                action_battery > threshold
            ]
            action_battery = np.select(conditions, [-1, 1], default=0)

        energy = action_battery * self.max_charge
        updated_soc = max(self.min_soc,
                          min(self.max_soc, (self.current_soc * self.capacity + energy * 15 / 60) / self.capacity))

        self.energy_change = (updated_soc - self.current_soc) * self.capacity   # if charge, positive, if discharge, negative
