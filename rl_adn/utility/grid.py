...
class GridTensor:
    """
     ...
     """
    def __init__(self, node_file_path: str = None, lines_file_path: str = None,
                 *, s_base: int = 1000,  # kVA - 1 phase
                 v_base: float = 11,  # kV - 1 phase
                 iterations: int = 100, tolerance: float = 1e-5, from_file=True, nodes_frame: pd.DataFrame = None, lines_frame: pd.DataFrame = None,
                 numba=True, gpu_mode=False):
    ...
    def run_pf_tensor(self,
        ...
        available_node_num = active_power.shape[0]
        if (active_power is not None) and (reactive_power is not None):
            print('ok')
            ...
            reactive_power = np.zeros((1, available_node_num))
            # print('zhong')

        self.ts_n = available_node_num  # Time steps to be simulated
        if flat_start:
        ...
        n_steps = S_nom.shape[0]
        n_nodes = available_node_num
        # n_nodes = self.bus_info.shape[0]
